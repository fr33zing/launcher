                        ...
                        db.getPayloadByNodeId(node.kind, node.nodeId).notNull(node).let { payload ->
                            payload.castOrNull<Reference>()?.targetId?.let { targetId ->
                                ReferenceFollowingNodePayloadState(
                                    ...
                                    targetState =
                                        run {
                                            val targetNode = db.nodeDao().getNodeById(targetId).notNull()
                                            val targetPayload =
                            ...
                            } ?: ReferenceFollowingNodePayloadState(NodePayloadState(node, payload), null)
                        }
                    }
            }
        }
        ...
        fun saveChanges() {
            ...
            val fixedNodes =
                reorderableNodes!!
                    ...
                    .also { states ->
                        states.forEachIndexed { index, state -> state.underlyingState.node.order = index }
                    }
