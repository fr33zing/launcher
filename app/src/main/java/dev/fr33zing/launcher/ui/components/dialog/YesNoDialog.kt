...
fun YesNoDialog(
    ...
    BaseDialog(visible, icon, onDismissRequest = onDismissRequest) { padding ->
        ...
        Column(modifier = Modifier.width(IntrinsicSize.Max).padding(vertical = padding)) {
            CompositionLocalProvider(LocalNodeDimensions provides rememberNodeDimensions()) {
                Option(visible, noText, noColor, noIcon, onNo)
                Option(visible, yesText, yesColor, yesIcon, onYes)
            }
        }
    }
}
