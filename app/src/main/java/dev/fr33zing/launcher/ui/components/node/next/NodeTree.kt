...
@Composable
fun paddingAndShadowHeight(): Pair<Dp, Dp> {
...
@Composable
fun NodeTree(items: List<TreeNodeState>, lazyListState: LazyListState = rememberLazyListState()) {
    ...
    Box(
        ...
        CompositionLocalProvider(LocalNodeDimensions provides createLocalNodeDimensions()) {
            LazyColumn(
                ...
                items(
                    items.map { Pair(it, progress(it.nodePayload.node.nodeId)) },
                    key = { it.first.nodePayload.node.nodeId },
                    contentType = { it.first.nodePayload.node.kind }
                ) { (state, progress) ->
                    NodeRow(
                        state,
                        features = NodeRowFeatures.All,
                        modifier =
                            Modifier.graphicsLayer {
                                translationY =
                                    (1 - progress.value) * APPEAR_ANIMATION_TRANSLATE_DP.dp.toPx()
